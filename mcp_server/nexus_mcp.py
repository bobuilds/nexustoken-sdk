...
This MCP Server exposes the Nexus task trading platform as native AI tools.
When installed, any MCP-compatible AI (Claude Desktop, Cursor, etc.) gains
two powerful new tools:

  1. nexus_create_task  — Delegate work to the Nexus network (demand side)
...
# Config from env
BASE_URL = os.getenv("NEXUS_BASE_URL", "http://localhost:8000").rstrip("/")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if ver_lt(MCP_VERSION, latest_ver):
                logger.warning(f"Nexus MCP Server {latest_ver} available (you have {MCP_VERSION})")
...
TOOLS = [
    Tool(
        ...
        description=(
            "Delegate a JSON extraction task to the Nexus Bot-to-Bot task network. "
    ...
    Tool(
        ...
        description=(
            "Check the status of a Nexus task or your account balance. "
    ...
    Tool(
        ...
        description=(
            "Browse available tasks on the Nexus network and optionally bid on one. "
    ...
    Tool(
        ...
        description=(
            "Submit your work result for a task you won on the Nexus network. "
...
async def main():
    if not API_KEY:
        logger.error("NEXUS_API_KEY environment variable is required")
        sys.exit(1)
    logger.info(f"Nexus MCP Server v{MCP_VERSION} starting (base_url={BASE_URL})")
    check_version()
